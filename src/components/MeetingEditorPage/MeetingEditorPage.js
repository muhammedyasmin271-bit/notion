...
const MeetingEditorPage = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      // Close block menu if clicking outside the dropdown container (which includes the button)
      // Only close on actual clicks, not on mouse movements
      if (showBlockMenu) {
        const menuRef = blockMenuRefs.current[showBlockMenu];
        if (menuRef && !menuRef.contains(event.target)) {
          // Also check if click is not on the button itself or any button in the menu
          const clickedButton = event.target.closest('button');
          const isMenuButton = clickedButton && (
            clickedButton.getAttribute('title') === 'Add block' ||
            clickedButton.closest('[class*="absolute left-0 top-8"]') // Inside the dropdown menu
          );
          if (!isMenuButton) {
      ...
      // Close line menu if clicking outside the dropdown container (which includes the button)
      if (showLineMenu) {
  ...
  const renderBlockContent = (block, index) => {
    ...
    const getInputClassName = () => {
      const baseClass = `w-full px-0 sm:px-3 py-0.5 sm:py-2 bg-transparent ${isDarkMode ? 'text-white placeholder-gray-400' : 'text-gray-900 placeholder-gray-500'} focus:outline-none border-none leading-relaxed font-inter`;
    ...
    if (block.type === 'bullet') {
      return (
        <div className="flex items-start gap-2">
          <span className={`mt-0.5 text-lg leading-none ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>•</span>
          <textarea
            ref={(el) => inputRefs.current[block.id] = el}
    ...
    if (block.type === 'numbered') {
      ...
      return (
        <div className="flex items-start gap-2">
          <span className={`mt-0.5 min-w-[20px] ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{number}.</span>
          <textarea
            ref={(el) => inputRefs.current[block.id] = el}
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-[#141414] text-white' : 'bg-white text-gray-900'} relative`}
      style={isDarkMode ? { backgroundColor: '#141414' } : {}}
    >
      {/* Delete and Save buttons - Top Right Corner */}
      {isOwner && (
        <div className="fixed top-4 sm:top-6 right-2 sm:right-6 flex items-center gap-2 z-30">
          {!isNewMeeting && (
            <button
              onClick={handleDelete}
              className="flex items-center justify-center gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-lg font-medium transition-all bg-red-600 hover:bg-red-700 text-white text-sm sm:text-base"
            >
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex items-center justify-center gap-2 px-3 sm:px-6 py-2 sm:py-2.5 rounded-lg font-medium transition-all bg-blue-600 hover:bg-blue-700 disabled:bg-gray-700 text-white text-sm sm:text-base"
          >
            {isSaving ? (
            ...
            <span className="hidden sm:inline">{isSaving ? 'Saving...' : (isNewMeeting ? 'Create Meeting' : 'Save Changes')}</span>
            <span className="sm:hidden">{isSaving ? 'Saving...' : (isNewMeeting ? 'Create' : 'Save')}</span>
          </button>
        </div>
      )}
      ...
      >
        ...
        <div className="max-w-full mx-auto pt-2 pb-20 sm:pb-6 overflow-y-auto px-0 sm:px-2">
          {loadingError && (
          ...
          <div className="w-full">
            <div>
              <div className={`rounded-lg pt-0 pb-0 px-0 sm:px-2 ${isDarkMode ? 'bg-[#141414]' : 'bg-white'}`}
                style={isDarkMode ? { backgroundColor: '#141414' } : {}}
              >
                <input
                  type="text"
                  ...
                  readOnly={!isOwner}
                  className={`w-full px-0 sm:px-2 py-3 text-2xl sm:text-4xl ml-0 sm:ml-0 rounded-lg transition-colors ${isDarkMode ? 'bg-[#141414] text-white' : 'bg-white text-gray-900'} placeholder-gray-400 focus:outline-none ${!isOwner ? 'cursor-not-allowed opacity-75' : ''}`}
                  style={isDarkMode ? { backgroundColor: '#141414' } : {}}
                />
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <select
                              ...
                              disabled={!isOwner}
                              className={`px-3 py-2 rounded-lg text-sm focus:outline-none w-40 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-900'}`}
                            >
                                      ...
                                      return (
                                        <div
                                          ...
                                          className={`w-full flex items-center px-3 py-2.5 rounded-lg cursor-pointer transition-all duration-200 ${isSelected
                                            ...
                                            : `${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'} hover:shadow-sm`
                                            }`}
                                        >
                                          <div className="mr-3">
                                            {isSelected ? (
                                          ...
                                          <div className="flex items-center flex-1">
                                            ...
                                            <div className="flex-1">
                                              <div className={`font-medium text-sm ${isDarkMode ? 'text-white' : 'text-black'}`}>{user.name}</div>
                          ...
                          <div className={`flex items-center gap-0 sm:gap-1 transition-opacity duration-200 absolute -left-6 top-0 z-10 sm:relative sm:left-auto sm:top-auto sm:z-auto sm:mr-1 sm:gap-1 ${currentBlockId === block.id ? 'opacity-100' : 'opacity-0 sm:group-hover:opacity-100'} ${showBlockMenu === block.id || showLineMenu === block.id ? 'opacity-100' : ''}`}>
                            ...
                            >
                              ...
                              {showBlockMenu === block.id && (
                                <div 
                                  className={`absolute left-0 top-8 w-64 sm:w-72 max-h-96 overflow-hidden border rounded-lg shadow-xl z-50 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}
                                  style={{ pointerEvents: 'auto', opacity: '1 !important' }}
                                >
                                  <div className={`p-3 border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                                    <div className={`flex items-center gap-2 text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                                      <Plus className="w-4 h-4" />
                                  ...
                                  <div className={`max-h-80 overflow-y-auto scrollbar-thin ${isDarkMode ? 'scrollbar-thumb-gray-600 scrollbar-track-gray-800' : 'scrollbar-thumb-gray-400 scrollbar-track-gray-200'}`}>
                                    <div className="p-2 space-y-1">
                                      <div className={`text-xs px-2 py-1 font-medium ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>BASIC BLOCKS</div>
                                    ...
                                    <button onClick={() => { const newBlocks = [...blocks]; newBlocks[index] = { ...block, type: 'heading1' }; setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                      <Hash className="w-4 h-4" /> Heading 1
                                    </button>
                                    <button onClick={() => { const newBlocks = [...blocks]; newBlocks[index] = { ...block, type: 'heading2' }; setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                      <Hash className="w-4 h-4" /> Heading 2
                                    </button>
                                    <button onClick={() => { const newBlocks = [...blocks]; newBlocks[index] = { ...block, type: 'heading3' }; setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                      <Hash className="w-4 h-4" /> Heading 3
                                    </button>

                                    <div className={`text-xs px-2 py-1 font-medium mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>LISTS</div>
                                    ...
                                    <button onClick={() => { const newBlocks = [...blocks]; newBlocks.splice(index + 1, 0, { id: `block-${Date.now()}`, type: 'toggle', content: '▶ ', style: {}, expanded: false }); setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                      <ArrowRight className="w-4 h-4" /> Toggle List
                                    </button>

                                    <div className={`text-xs px-2 py-1 font-medium mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>MEDIA</div>
                                    ...
                                    <button onClick={() => { const newBlocks = [...blocks]; newBlocks.splice(index + 1, 0, { id: `block-${Date.now()}`, type: 'code', content: '', style: {} }); setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                    ...
                                    <div className={`text-xs px-2 py-1 font-medium mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>FUNCTIONAL</div>
                                    ...
                                    <button onClick={() => { const newBlocks = [...blocks]; newBlocks.splice(index + 1, 0, { id: `block-${Date.now()}`, type: 'priority', content: '', priorityLevel: 'medium', style: {} }); setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                      <Star className="w-4 h-4 text-yellow-400" /> Priority
                                    </button>

                                    <div className={`text-xs px-2 py-1 font-medium mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>ADVANCED</div>
                                    <button onClick={() => {
                                    ...
                                    <div className={`text-xs px-2 py-1 font-medium mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>CALLOUTS</div>
                                    ...
                                    <button onClick={() => { const newBlocks = [...blocks]; newBlocks.splice(index + 1, 0, { id: `block-${Date.now()}`, type: 'callout', calloutType: 'success', content: '', style: {} }); setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                      ...
                                      <div className={`text-xs px-2 py-1 font-medium mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>MEDIA</div>
                                      <button onClick={() => { const newBlocks = [...blocks]; newBlocks.splice(index + 1, 0, { id: `block-${Date.now()}`, type: 'image', content: '', url: '', alt: '', style: {} }); setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                        <Image className="w-4 h-4 text-green-400" /> Image
                                      </button>
                                      <button onClick={() => { const newBlocks = [...blocks]; newBlocks.splice(index + 1, 0, { id: `block-${Date.now()}`, type: 'video', content: '', url: '', style: {} }); setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                        <Video className="w-4 h-4 text-red-400" /> Video
                                      </button>
                                      <button onClick={() => { const newBlocks = [...blocks]; newBlocks.splice(index + 1, 0, { id: `block-${Date.now()}`, type: 'file', content: '', fileName: '', style: {} }); setBlocks(newBlocks); setShowBlockMenu(null); }} className={`w-full flex items-center gap-2 px-3 py-2 text-left rounded transition-colors ${isDarkMode ? 'hover:bg-gray-700 text-white' : 'hover:bg-gray-100 text-black'}`}>
                                        <FileIcon className="w-4 h-4 text-gray-400" /> File
                                      </button>

                                      <div className={`text-xs px-2 py-1 font-medium mt-2 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>SPECIAL</div>
                            ...
                            >
                              ...
                              {showLineMenu === block.id && (
                                <div 
                                  className={`absolute left-0 top-8 w-36 sm:w-40 border rounded-lg shadow-xl z-50 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}
                                  style={{ pointerEvents: 'auto', opacity: '1 !important' }}
                                >
                                  <div className="p-2">
