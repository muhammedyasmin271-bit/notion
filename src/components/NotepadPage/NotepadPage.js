        ...
        >
					...
					<div className={`p-4 border-b ${isDarkMode ? 'border-gray-700/30' : 'border-gray-200/30'}`}>
						<div className="flex items-center justify-between mb-3">
							<button
								onClick={() => setIsSidebarOpen(false)}
								className="flex items-center gap-3 flex-1 text-left hover:opacity-80 transition-all duration-200 group cursor-pointer"
                ...
                >
							...
							<div className="flex items-center gap-1.5">
								<button
									onClick={() => setIsSidebarOpen(false)}
									className={`p-2 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95 ${isDarkMode ? 'hover:bg-gray-800/50 text-gray-400' : 'hover:bg-gray-100 text-gray-500'}`}
									aria-label="Close Sidebar"
									title="Close Sidebar (Press Escape)"
								>
									<X className="w-4 h-4" />
								</button>
								<button
                  ...
                  style={isDarkMode ? { backgroundColor: '#141414' } : {}}
						...
						<div className="relative">
							<Search className={`absolute left-3.5 top-1/2 transform -translate-y-1/2 w-4 h-4 z-10 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
							<input
								type="text"
					...
					<div className="flex-1 overflow-y-auto p-2.5 pb-20 md:pb-4">
						{loading ? (
							...
							<div className={`p-4 rounded-xl ${isDarkMode ? 'bg-red-900/20 border border-red-500/30 text-red-400' : 'bg-red-50 border border-red-200 text-red-600'}`}>
								...
								{filteredNotes.map(note => (
									...
									>
										<div className="flex justify-between items-center gap-2">
											<div className={`font-semibold text-sm truncate flex-1 leading-tight ${selectedNote === note._id ? (isDarkMode ? 'text-white' : 'text-gray-900') : (isDarkMode ? 'text-gray-200' : 'text-gray-800')}`}>
												{note.title || 'Untitled'}
											</div>
											<div className="flex items-center gap-0.5 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
												<button
													...
													className={`p-1 rounded transition-all duration-200 hover:scale-110 ${isDarkMode ? 'hover:bg-gray-700/50' : 'hover:bg-gray-200/50'}`}
												>
													{favorites.includes(note._id) ? (
														<Star className="w-3.5 h-3.5 text-yellow-500 fill-current" />
													) : (
														<Star className={`w-3.5 h-3.5 ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`} />
													)}
												</button>
												<button
													onClick={(e) => {
													...
													className={`p-1 rounded transition-all duration-200 hover:scale-110 ${isDarkMode ? 'hover:bg-red-900/30 text-gray-500 hover:text-red-400' : 'hover:bg-red-100 text-gray-400 hover:text-red-500'}`}
												>
										...
										{note.tags && note.tags.length > 0 && (
											<div className="flex flex-wrap gap-1 mt-1.5">
												...
												{note.tags.length > 2 && (
													<span className={`px-1.5 py-0.5 text-xs rounded-md ${isDarkMode ? 'bg-gray-700/50 text-gray-400' : 'bg-gray-100 text-gray-500'}`}>
														+{note.tags.length - 2}
													</span>
												)}
											</div>
