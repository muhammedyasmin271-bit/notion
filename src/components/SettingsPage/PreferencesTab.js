...
const PreferencesTab = () => {
  ...
  useEffect(() => {
    const loadPreferences = async () => {
      try {
        ...
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL || 'https://notion-l9ti.onrender.com'}/api/users/profile`, {
  ...
  useEffect(() => {
    const loadSMSUsage = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`${process.env.REACT_APP_BACKEND_URL || 'https://notion-l9ti.onrender.com'}/api/notifications/sms-usage`, {
          headers: { 'x-auth-token': token }
        });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.patch(`${process.env.REACT_APP_BACKEND_URL || 'https://notion-l9ti.onrender.com'}/api/notifications/email-settings`, 
        { emailNotifications: notifications.emailNotifications },
        { headers: { 'x-auth-token': token } }
      );
      
      // Save SMS notification preference
      await axios.patch(`${process.env.REACT_APP_BACKEND_URL || 'https://notion-l9ti.onrender.com'}/api/notifications/sms-settings`, 
        { smsNotifications: notifications.smsNotifications },
        { headers: { 'x-auth-token': token } }
      );

      // Save quiet hours settings
      await axios.patch(`${process.env.REACT_APP_BACKEND_URL || 'https://notion-l9ti.onrender.com'}/api/notifications/quiet-hours`,
        quietHours,
        { headers: { 'x-auth-token': token } }
      );
  ...
  const handleTestEmail = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL || 'https://notion-l9ti.onrender.com'}/api/notifications/test-email`, 
        {},
        { headers: { 'x-auth-token': token } }
      );
  ...
  const handleTestSms = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL || 'https://notion-l9ti.onrender.com'}/api/notifications/test-sms`, 
        {},
        { headers: { 'x-auth-token': token } }
      );
